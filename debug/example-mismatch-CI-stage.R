...
## summary(currentGSD.decision[[1]][[iMeth]])
...
DelayedGSD.options(FCT.p_value="FinalPvalue", continuity.correction = 1)
currentGSD.final[[iMeth]] <- update(currentGSD.interim[[2]][[iMeth]], delta = lmmF, trace = FALSE)
summary(currentGSD.final[[iMeth]])
## estimate   lower   upper  p.value
##  0.42486 0.00249 0.84036 0.024343

DelayedGSD.options(FCT.p_value="FinalPvalue2", continuity.correction = 2)
currentGSD.final2 <- update(currentGSD.interim[[2]][[iMeth]], delta = lmmF, trace = FALSE)
summary(currentGSD.final2)
## debug(DelayedGSD:::FinalCI)
## estimate    lower  upper  p.value
...
## ** debug
undebug(DelayedGSD:::FinalCI)

fff <- function(delta){
    do.call(DelayedGSD:::FinalPvalue2, list(Info.d=c(10.85916039, 17.63486364, 22.41020084),
                                            Info.i=c(7.68364329, 14.07480953),
                                            ck=c(1.39502083, 1.72345518, 2.03643431),
                                            ck.unrestricted=c(1.39502083, 1.72345518, 2.03643431),
                                            lk=c(-0.37404736, 0.71373586),
                                            uk=c(2.79415089, 2.44790563),
...
ls.optim <- lapply(1:10, function(x){
    cat("\n")
    stats::optimise(function(x){
        y <- fff(x)
        cat(x, ": ",y,"\n")
        return((y - 0.05/2)^2)},
        lower = -0.4116661,
        upper = 0.538916,
        tol = 1e-10)})
sapply(ls.optim,"[[","minimum")


seqX <- seq(-0.01,0.01,0.0001)
plot(seqX,sapply(seqX,fff)-0.05/2)
##----------------------------------------------------------------------
